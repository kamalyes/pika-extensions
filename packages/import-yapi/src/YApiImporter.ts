...
import { getDataType } from '../../import-postman/src/utils'
...
export class YApiImporter {
  ...
  constructor(data) {
    this.yapiData = data
    this.pikaData = this.transformToPika(this.yapiData)
  }

  transformToPika(data): ImportProjectDto {
    // 项目名
  ...
  transformApis(items: any): ImportProjectDto['collections'] {
    return (
      items?.map((item) => {
        ...
        const contentType = this.handleRequestBodyType(
          item?.req_body_type,
          item.req_body_other
        )
        const resContentType = this.handleRequestBodyType(
          item?.res_body_type,
          item.res_body
        )
        return {
          ...
          responseList: [
            {
              ...
              responseParams: {
                // 没有header?
                headerParams: [],
                bodyParams: this.handleResponseBody(
                  item?.res_body,
                  resContentType
                )
                // bodyParams: []
      ...
      return (
        Object.entries(jsonSchema?.properties || {}).map(([key, value]) => ({
          ...
          childList:
            // 对象处理
            value.type === 'object'
              ? this.handleJsonBody(value, paramType)
              : // 数组处理
              value.type === 'array' &&
                value.items &&
