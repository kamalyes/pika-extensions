...
const PikaParams2Eoapi = (params) => {
  return params.map((val) => {
    if (val.children) val.children = PikaParams2Eoapi(val.children)
    return {
      name: val.name,
...
const Pika2Eoapi = (origin) => {
  const result = {
    uri: origin.uri,
    requestBody: PikaParams2Eoapi(origin.requestParams.bodyParams),
    queryParams: PikaParams2Eoapi(origin.requestParams.queryParams),
    restParams: PikaParams2Eoapi(origin.requestParams.restParams),
    responseBody: PikaParams2Eoapi(
...
export const sercurityCheck = async (model) => {
  ...
  const serverUrl = pc?.getExtensionSettings('pika-opendlp.serverUrl')
  if (serverUrl) {
    const Grpc = pc.gRPC
    console.log('params', params)

    const modal = pc?.modalService.create({
      nzTitle: 'API 敏感词',
      nzBodyStyle: {
        maxHeight: '70vh',
        overflow: 'hidden'
