...
export class APIFoxImporter {
  ...
  constructor(data) {
    ...
    this.pikaData = this.transformToPika(this.apifoxData)
  }
        ...
        if (!item.api) {
          ...
          return {
            ...
            requestParams: {
              ...
              bodyParams: this.handleBodyParams(
                api?.requestBody,
                PARAM_TYPE.input
              )
              // bodyParams: []
            ...
            responseList: [
              {
                isDefault: 1,
                contentType: this.handleResponseBodyType(
                  api.responses?.[0] || {}
                ),
                responseParams: {
                  // 没有header
                  headerParams: [],
                  bodyParams: this.handleJsonBody(
                    api.responses?.[0]?.jsonSchema || {},
                    PARAM_TYPE.output
                  )
                  // bodyParams: []
                }
