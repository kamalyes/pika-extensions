...
export class OpenAPIParser {
  ...
  schema2PikaEditBody(
    ...
    if (this.is$ref(schema)) {
      ...
      return schemaObject
        ? this.schema2PikaEditBody(schemaObject).concat(
  ...
  generateBody(
    ...
    if (this.is$ref(body)) {
      const [ref] = this.get$Ref(body)
      const schemaObject = this.getSchemaBy$ref(ref)
      return schemaObject ? this.schema2PikaEditBody(schemaObject) : []
    } else if (body?.content) {
      const media = Object.values(body.content).at(0)
      return media?.schema ? this.schema2PikaEditBody(media?.schema) : []
    }
    return this.schema2PikaEditBody(body)
  }

  generateResponseBody(responses: OpenAPIV3.ResponsesObject) {
    const resObj = this.getResponseObject(responses)
    if (resObj?.content) {
      return this.schema2PikaEditBody(
