...
export const listToTreeHasLevel = (
  ...
  list = list.map((item) => {
    ...
    return Object.keys(item).reduce(
      (pre, itemKey) =>
        uselessKeys.includes(itemKey)
          ? pre
          : { ...pre, [itemKey]: item[itemKey] },
      {}
    )
  })
...
export const treeToListHasLevel = (
  tree,
  opts: { listDepth: number; mapItem?: (val) => object } = { listDepth: 0 }
) => {
...
export const listToTree = (
  list: Array<GroupTreeItem>,
  tree: Array<GroupTreeItem>,
 ...
 */
export const findDataInTree = (
  // rome-ignore lint/suspicious/noExplicitAny: <explanation>
  _data: any,
  value,
  { nodeId = 'nodeKey', id, key }
): any => {
...
export const getExpandGroupByKey: (component, key) => string[] = (
  component,
  key
) => {
  if (!component) {
